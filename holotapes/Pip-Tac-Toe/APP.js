...
function drawBoard() {
  ...
  for (let y = 0; y < 3; y++) {
    for (let x = 0; x < 3; x++) {
      let val = board[y][x] || " ";
      let drawX = offsetX + x * (cellWidth + spacing) + cellWidth / 2 - 9;
      let drawY = offsetY + y * (cellHeight + spacing) + cellHeight / 2 - 0;
...
function showMainMenu() {
  ...
  if (typeof ENC1_PRESS !== "undefined") {
    leftWheelPressWatch = setWatch(function () {
      playSound(snd_Play);
      inMenu = false;
  ...
  function drawMenu() {
    ...
    bC.drawString(title, (screenWidth - bC.stringWidth(title)) / 2, 50);
    ...
    for (let i = 0; i < menuOptions.length; i++) {
      ...
      if (i === menuSelection) {
        bC.drawRect(156, y - 16, 156 + bC.stringWidth(opt) + 8, y + 16);      }

      bC.drawString(opt, 160, y);
    }

    bC.setFont("6x8", 2);
    bC.drawString("Left Wheel Vertical/Select", (screenWidth - bC.stringWidth(title)) / 4, 240);
    bC.drawString("Right Wheel Horizontal", (screenWidth - bC.stringWidth(title)) / 3.5, 270);
